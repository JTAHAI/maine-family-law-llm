...
if FastAPI is not None:
    ...
    def _attach_record_open_capabilities(case_root: Path | None, citations: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Attach opaque open/inspect capabilities only to indexed private records."""
        if case_root is None:
            return citations
        try:
            indexed_ids = {str(row.get("evidence_id") or "") for row in load_case_search_records(case_root)}
        except Exception:
            return citations
    ...
    def _group_record_cards(case_root: Path, citations: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for citation in citations:
            ...
            card = grouped.setdefault(group_id, {
                ...
                "viewer_kind": _record_viewer_kind(Path(_safe_record_basename(citation)).suffix.lower(), mimetypes.guess_type(_safe_record_basename(citation))[0] or "application/octet-stream"),
                "match_count": 0,
                "pages": [],
                "snippets": [],
            })
    ...
    async def local_security_headers(request: Request, call_next):  # type: ignore[no-untyped-def]
        ...
        response.headers["X-Frame-Options"] = "SAMEORIGIN" if request.url.path.startswith("/api/records/open/") else "DENY"
    ...
    def _finalize_family_response(result: dict[str, Any], payload: AskRequest) -> dict[str, Any]:
        ...
        raw_citations = _attach_record_open_capabilities(active_case_root(), list(result.get("citations") or []))
    ...
    def _resolve_record_capability(token: str, page: int = 0) -> dict[str, Any]:
        ...
        candidate["source_locator"] = locator_without_page or str(root.get("source_locator") or path.name)
        try:
        ...
        for candidate_row in rows:
            ...
            if candidate_base == target_locator and (not resolved_page or candidate_page in {0, resolved_page}):
    ...
    def _safe_email_preview(resolved: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return {"headers": {}, "body": "The email could not be parsed safely.", "attachments": [], "parse_status": "unreadable"}
        headers = {
        ...
        for part in message.walk():
            ...
            if filename:
                ...
                attachments.append({
                    ...
                    "viewer_kind": _record_viewer_kind(Path(filename).suffix.lower(), content_type),
                })
                continue
            ...
            if content_type == "text/html":
                content = parse_bytes(str(content).encode("utf-8"), suffix=".html", locator="message.html").text
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(data)) as archive:
                ...
                for info in archive.infolist():
                    ...
                    members.append({
                        "filename": safe_name,
                        "member_locator": "!".join(Path(part.replace("\\", "/")).name for part in member_locator.split("!")),
    ...
    def _safe_table_preview(data: bytes, suffix: str) -> dict[str, Any]:
        ...
        for row in reader:
            ...
            if len(rows) >= 120:
                break
        return {"rows": rows, "rows_truncated": len(decoded.splitlines()) > len(rows), "column_limit": 40}
    ...
    def _record_inspection_payload(resolved: dict[str, Any]) -> dict[str, Any]:
        ...
        page_count = int(row.get("page_count") or root.get("page_count") or parser_metadata.get("page_count") or 0)
        ...
        if viewer_kind == "email":
            ...
            preview = {
                "ocr_text": str(row.get("text_content") or row.get("text_excerpt") or "")[:_RECORD_PREVIEW_TEXT_LIMIT],
                "ocr_status": str(row.get("ocr_status") or root.get("ocr_status") or "unknown"),
            }
        else:
            preview = {"message": "A safe in-app rendering is not available for this file type. You can open or download the verified original."}
        ...
        return {
            ...
            "source_type": str(row.get("source_type") or root.get("source_type") or suffix.lstrip(".") or "record"),
            "parser_status": str(row.get("parser_status") or root.get("parser_status") or "unknown"),
    ...
    def open_record(token: str, page: int = 0, download: bool = False):  # type: ignore[no-untyped-def]
        ...
        force_attachment = bool(download or active_content or suffix in {".eml", ".mbox", ".mbx", ".docx", ".xlsx", ".pptx", ".zip"})
        headers = {
    ...
    def document_workspace_status() -> dict[str, Any]:
        try:
            ...
            return status | {
                ...
                "explicit_confirmation_required": bool(status.get("destructive_actions_approval_gated")),
        ...
        try:
            ...
            return {
                ...
                "actions": [
                    ...
                    {"type": "export_document", "format": "docx", "document_id": document["document_id"]},
                ],
                "review_required": True,
                "filing_ready": False,
            }
    ...
    def document_workspace_export(document_id: str, format: str = "txt"):  # type: ignore[no-untyped-def,redefined-builtin]
        try:
            ...
            if requested in {"txt", "md"}:
                ...
                slug = re.sub(r"[^A-Za-z0-9._-]+", "-", str(document["title"])).strip("-.")[:80] or "document"
    ...
    def document_workspace_artifact(artifact_id: str):  # type: ignore[no-untyped-def]
        try:
            ...
            if not re.fullmatch(r"[a-f0-9]{32}", artifact_id):
                raise DocumentWorkspaceError("artifact_not_found", "The artifact was not found.", status_code=404)
