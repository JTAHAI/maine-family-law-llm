...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .result-toolbar { display: flex; justify-content: space-between; gap: 0.75rem; align-items: center; margin-top: 1.25rem; flex-wrap: wrap; }
    .badges { display: flex; gap: 0.5rem; flex-wrap: wrap; }
    .badge { border: 1px solid var(--line); border-radius: 999px; padding: 0.25rem 0.55rem; font-size: 0.82rem; color: var(--muted); }
...
<body>
  ...
  <script>
    ...
    function renderBadges(payload) {
      ...
      if (payload.answer_style) badges.push(`<span class="badge">${escapeHtml(payload.answer_style.replace('_', ' '))}</span>`);
      if (payload.safety && payload.safety.category) badges.push(`<span class="badge">safety: ${escapeHtml(payload.safety.category)}</span>`);
      badges.push(`<span class="badge">review required</span>`);
      ...
      try {
        const res = await fetch('/ask', {
          ...
          body: JSON.stringify({
            ...
            matter_context: matterContext.value
          })
        });
    ...
    clearButton.addEventListener('click', () => { question.value = ''; matterContext.value = ''; answer.textContent = 'Ask a question to test the local source-grounded workbench.'; answerBadges.innerHTML = '<span class="badge">waiting</span>'; sourceCards.textContent = 'No source cards yet.'; });
