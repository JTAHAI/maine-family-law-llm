...
def discover_case_roots(case_build_root: Path | None = None) -> list[Path]:
    root = case_build_root or default_case_build_root()
    if not root.exists():
        return []
    ...
    for child in sorted(root.iterdir(), key=lambda path: path.name.lower()):
        if not child.is_dir():
            continue
        if (child / "15_PROOF_VALIDATION" / "CASE_BUILD_PROOF.json").exists():
...
def remember_active_case(case_root: Path | None) -> Path:
    state = load_workspace_state()
    state["last_active_case_root"] = str(case_root) if case_root else ""
    state["updated_at"] = utc_now()
    return save_workspace_state(state)


def load_active_case() -> Path | None:
    value = str(load_workspace_state().get("last_active_case_root", "")).strip()
    if not value:
        return None
    path = Path(value)
