    ...
    function renderLatestAnswer(payload) {
      if (payload?.direct_record_search) {
        answer.innerHTML = `<div class="answer-body compact-search-result"><section class="answer-section"><h3>Local record search</h3>${renderParagraphBlocks(String(payload.answer || 'Search completed.'))}</section><section class="answer-section source-lane-summary"><p><strong>Source lane:</strong> private matter records only. ${escapeHtml(payload.source_card_count || 0)} source card(s) returned.</p><p class="muted">This direct search does not substitute a Maine-law answer or make a legal conclusion.</p></section></div>`;
        return;
      }
    ...
    async function loadInventoryStatus() {
      ...
      try {
        ...
        const ocrCandidates = Number(payload.ocr_candidates || 0);
        const searchable = Number(payload.searchable_records || 0);
        if (ocrCandidates) {
          inventoryStatus.textContent = `Local inventory: Partially searchable · ${Number(payload.records || 0).toLocaleString()} records · ${searchable.toLocaleString()} searchable · OCR choice required for ${ocrCandidates.toLocaleString()} scanned page(s).`;
        } else if (warnings) {
