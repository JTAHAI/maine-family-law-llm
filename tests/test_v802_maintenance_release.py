"""Current release identity advances without enabling failed specialist candidates."""
...
def test_current_scope_versions_and_specialist_exclusion():
    scope = json.loads((ROOT / "configs/v901_release_scope.json").read_text())
    ...
    assert VERSION == scope["release"] == truth["release"]["product_version"] == "9.0.1"
    assert PACKAGE_VERSION == scope["package_version"] == identity["package_version"] == "9.0.1.0"
    assert BUILD_NUMBER == 57
    assert tuple(map(int, PACKAGE_VERSION.split("."))) > (8, 0, 1, 0)
    assert truth["release"]["release_scope"] == "configs/v901_release_scope.json"
    assert scope["new_bundled_legal_model_ids"] == []
    assert scope["new_public_feature_ids"] == ["optional_local_qwen_source_review", "consent_bound_local_ai_setup"]
