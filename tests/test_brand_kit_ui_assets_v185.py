...
def test_v185_local_workbench_uses_brand_assets_and_visible_beautiful_shell() -> None:
    ...
    assert 'UI v1.85 FOCAF brand-kit asset integration' in html
    assert 'Brand assets loaded from /brand-assets' in html
    assert "window.__MFL_WORKBENCH_UI_VERSION = '1.85.0-branded-ui-asset-integration'" in html
...
def test_v185_runtime_diagnostics_reports_brand_asset_mount() -> None:
    ...
    assert payload["version"] == "1.85.0"
    assert payload["ui_version"] == "1.85.0-branded-ui-asset-integration"
