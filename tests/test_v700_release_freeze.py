...
def test_v700_canonical_versions_and_about_surface_are_consistent() -> None:
    assert VERSION == "7.0.0"
    assert PACKAGE_VERSION == "7.0.0.0"
    assert BUILD_NUMBER == 52
    assert UI_VERSION == "7.0.0-ga-b52"
    assert UI_FOOTER_LABEL == "v7.0.0"
    assert 'version = "7.0.0"' in (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    html = render_local_workbench_html()
    assert "7.0.0" in html
    assert "7.0.0.0" in html
    assert "6.0.4-extended-hardening-full-source.zip" not in html


def test_v700_store_identity_and_manifest_contract_are_unchanged_except_version() -> None:
